 ...
 * - View mode toggle (Average / Total)
 * - Sortable columns
 * - Favorite players
...
export default function SimplePlayerTable({ players, isLoading }: SimplePlayerTableProps) {
  ...
  useEffect(() => {
    logger.info('SimplePlayerTable', 'Component rendered', {
  ...
  }, [players, viewMode, positionFilter, teamFilter, sortField, sortDirection]);

  // ============================================
  // EVENT HANDLERS
  // ============================================
  
  const handleViewModeChange = (newMode: "average" | "total") => {
    logger.info('SimplePlayerTable', 'View mode changed', {
      from: viewMode,
      to: newMode
    });
    
    setViewMode(newMode);
    if (newMode === "total" && sortField === "averagePoints") {
      setSortField("totalPoints");
    } else if (newMode === "average" && sortField === "totalPoints") {
      setSortField("averagePoints");
    }
  };
  ...
  const handleSort = (field: keyof SimplePlayer) => {
    ...
    logger.info('SimplePlayerTable', 'Sort changed', {
      field,
      direction: sortField === field ? (sortDirection === "asc" ? "desc" : "asc") : "desc"
    });
  };
  ...
  const toggleFavorite = (playerId: number) => {
    ...
    if (newFavorites.has(playerId)) {
      ...
      logger.info('SimplePlayerTable', 'Player unfavorited', { playerId });
    } else {
      newFavorites.add(playerId);
      logger.info('SimplePlayerTable', 'Player favorited', { playerId });
    }
      ...
      <Card className="bg-gray-800 border-gray-700">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <select
              value={viewMode}
              onChange={(e) => handleViewModeChange(e.target.value as "average" | "total")}
              className="px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white"
              ...
              <TableHeader>
                <TableRow className="border-gray-700">
                  ...
                  {viewMode === "average" ? (
                    ...
                    <SortHeader field="totalPoints">Total</SortHeader>
                  )}
                  <SortHeader field="roundsPlayed">Games</SortHeader>
              ...
              <TableBody>
                {filteredAndSortedPlayers.map((player) => (
                  ...
                  >
                    ...
                    <TableCell className="text-gray-300">{formatCurrency(player.price)}</TableCell>
                    ...
                    <TableCell className="text-gray-300">{player.roundsPlayed || 0}</TableCell>
                    <TableCell className="text-gray-300">{player.lastScore || '-'}</TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-1">
                        ...
                        <span className={
                          player.priceChange > 0 ? "text-green-500" :
                          player.priceChange < 0 ? "text-red-500" : "text-gray-400"
                        }>
                          {formatCurrency(Math.abs(player.priceChange))}
                        </span>
                      </div>
                    </TableCell>
                    ...
                    {/* Ownership % */}
                    <TableCell className="text-gray-300">
                      {(player.selectionPercentage || 0)?.toFixed(1)}%
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
